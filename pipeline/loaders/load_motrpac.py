...
def main(genes: list[str] | None = None):
    ...
    # Human gene symbol -> UniProt accession (from our seed proteins + DB)
    human_gene_to_acc = {g: a for a, g in SEED_PROTEINS.items()}
    # Also pull any proteins already in the DB
    for row in conn.execute("SELECT uniprot_accession, gene_symbol FROM protein"):
        human_gene_to_acc.setdefault(row[1].upper() if row[1] else "", row[0])

    # Determine target genes (None = all mapped genes, no filter)
    if genes is not None:
        target_genes = set(g.upper() for g in genes)
    else:
        target_genes = None  # no filter -- load everything that maps
    ...
    for _, row in unique_sites.iterrows():
        ...
        # Check if this gene is in our target set (if filtering)
        if target_genes is not None and human_gene.upper() not in target_genes:
            continue

        # Get UniProt accession from DB or R2H mapping
        acc = human_gene_to_acc.get(human_gene.upper())
        if not acc:
            # Try to get UniProt ID from the rat-to-human mapping
            human_uniprot = None
            for _, r2h_row in r2h_gene[r2h_gene["HUMAN_ORTHOLOG_SYMBOL"].str.upper() == human_gene.upper()].iterrows():
                hu = r2h_row.get("RAT_UNIPROT_ID")  # not human uniprot, skip
                break
            # We don't have a reliable human UniProt mapping outside our DB,
            # so skip proteins we can't map
            skipped_no_acc += 1
            continue
